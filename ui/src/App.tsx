...
import { Activity, ArrowLeft, ArrowRight, CircleHelp, Database, Download, Eye, FileJson, FlaskConical, KeyRound, Layers3, LogOut, Plus, RotateCcw, Search, Settings2, Sun, Moon } from "lucide-react";
...
import { catalogModels, copy, delta, exposed, keyImpact, members, modelImpact, origin, same, toggleModel, type Demo, type Group, type Key, type Model, type Policy } from "./demo";
...
export function App() {
  ...
  const requireLogin = () => {
    ...
    setLoginToken("");
    setLoginError(revision ? "Your session expired. Enter the admin token to continue." : "");
  };
  const load = async () => {
    if (busyRef.current) return;
    setLoading(true);
    try { applyWorkspace(await getWorkspace()); if (reconcileBlocked) { setReconcileBlocked(false); setNeedsReconcile(false); } }
    catch (error) { if (error instanceof ApiError && error.status === 401) requireLogin(); else setApiError(error instanceof ApiError && error.status === 403 ? "Access to the Registry panel was denied (403)." : error instanceof ApiError ? error.message : "Could not connect to Registry. Check the network and try again."); }
  ...
  const signIn = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!token) { setLoginError("Enter the admin token."); return; }
    setLoading(true);
    setLoginError("");
    setAdminToken(token);
    try {
      ...
      clearAdminToken();
      setLoginToken("");
      setLoginError(error instanceof ApiError && error.status === 401 ? "Invalid or expired admin token." : error instanceof ApiError && error.status === 403 ? "Access to the Registry panel was denied (403)." : "Could not connect to Registry. Check the network and try again.");
    } finally { setLoading(false); }
  };
  const signOut = () => {
    clearAdminToken();
  ...
  const savedSelection = key ? exposed(key.policy, data.groups, data.models) : [];
  const diff = delta(savedSelection, predicted);
  const isDirty = Boolean(key && draft && !same(key.policy, draft));
  const modelIsDirty = Boolean(modelDraft && modelBaseline.current && !same(modelDraft, modelBaseline.current));
  const groupIsDirty = Boolean(groupDraft && groupBaseline.current && !same(groupDraft, groupBaseline.current));
  const requestSignOut = () => {
    if (modelIsDirty || groupIsDirty || isDirty) setConfirm({ title: "Discard drafts and sign out?", text: "Unsaved changes will be lost.", confirmLabel: "Discard and sign out", cancelLabel: "Keep editing", action: signOut });
    else signOut();
  };
  useEffect(() => {
  ...
  const openModel = (model: Model, creating: boolean) => { modelBaseline.current = copy(model); setCreatingModel(creating); setModelError(""); setModelDraft(copy(model)); };
  const closeModel = () => {
    if (!modelDraft || busyRef.current) return;
    if (modelIsDirty) setConfirm({ title: "Discard model changes?", text: "Unsaved model details will be lost.", confirmLabel: "Discard changes", cancelLabel: "Keep editing", action: () => setModelDraft(null) });
  ...
  const closeGroup = () => {
    ...
    if (groupIsDirty) setConfirm({ title: "Discard group changes?", text: "Unsaved group selections will be lost.", confirmLabel: "Discard changes", cancelLabel: "Keep editing", action: () => setGroupDraft(null) });
  ...
  if (loginRequired) return <div className="flex min-h-dvh items-center justify-center bg-background p-4 sm:p-6"><Card className="w-full max-w-md p-6"><form onSubmit={signIn} className="space-y-5"><div className="space-y-2"><h1 className="text-xl font-semibold">Sign in to Registry</h1><p className="text-sm text-muted-foreground">Enter the panel admin token.</p></div><div className="space-y-2"><label htmlFor="registry-admin-token" className="text-sm font-medium">Admin token</label><Input id="registry-admin-token" type="password" autoComplete="current-password" autoFocus required spellCheck={false} value={loginToken} onChange={event => setLoginToken(event.target.value)} /></div>{loginError && <p role="alert" className="text-sm text-destructive">{loginError}</p>}<Button type="submit" className="w-full" disabled={loading}>{loading ? "Signing in…" : "Sign in"}</Button></form></Card></div>;
  ...
  return <TooltipProvider delayDuration={0}><SidebarProvider><CloseMobileSidebarOnRoute routeKey={`${page}/${pageId || ""}`} /><Sidebar collapsible="icon" className="border-none bg-transparent"><SidebarHeader className="px-4 pt-5 pb-3 group-data-[collapsible=icon]:px-2"><MobileSidebarLabel /><CollapsedSidebarExpand /><div className="flex items-center justify-between gap-2 group-data-[collapsible=icon]:hidden"><a href="#/models" onClick={e => { e.preventDefault(); go("models"); }} className="min-w-0"><img src={`${import.meta.env.BASE_URL}bifrost-logo.webp`} alt="Bifrost" className="h-[22px] max-w-[145px] object-contain object-left dark:hidden" /><img src={`${import.meta.env.BASE_URL}bifrost-logo-dark.webp`} alt="Bifrost" className="hidden h-[22px] max-w-[145px] object-contain object-left dark:block" /></a><SidebarTrigger className="size-7" /></div></SidebarHeader><SidebarContent><SidebarGroup><SidebarGroupLabel>WORKSPACE</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{nav.map(item => { const Icon = item.icon; return <SidebarMenuItem key={item.id}><SidebarMenuButton asChild isActive={page === item.id} tooltip={item.label}><a href={`#/${item.id}`} onClick={e => { e.preventDefault(); if (item.id === "catalog") setCatalogFocus(null); go(item.id); }} className={page === item.id ? "border border-primary/20 text-primary" : "border border-transparent"}><Icon className="size-4" /><span>{item.label}</span></a></SidebarMenuButton></SidebarMenuItem>; })}</SidebarMenu></SidebarGroupContent></SidebarGroup></SidebarContent><SidebarFooter className="p-4"><span className="text-xs text-muted-foreground group-data-[collapsible=icon]:hidden">Model Registry</span></SidebarFooter></Sidebar>
    <div className="flex h-dvh w-full min-w-0 flex-col"><header className="flex h-13 w-full shrink-0 items-center gap-2 px-3 pt-1 md:pr-3 md:pl-0"><SidebarTrigger className="md:hidden" /><h1 className="min-w-0 flex-1 truncate text-sm font-medium text-muted-foreground">{key && page === "keys" ? <><button onClick={() => go("keys")} className="text-muted-foreground hover:text-foreground">Virtual Keys</button><span className="mx-2 text-muted-foreground/50">/</span>{title}</> : title}</h1><Tooltip><TooltipTrigger asChild><button type="button" aria-label={`Help for ${title}`} className="inline-flex rounded-sm text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><CircleHelp className="size-4" /></button></TooltipTrigger><TooltipContent>{page === "models" ? "Explore models and provider accesses" : page === "groups" ? "Shared sets of models for keys" : page === "keys" ? "Compose and publish key catalogs" : page === "settings" ? "AI assistance and display preferences" : page === "catalog" ? "Review source data, references, and mappings" : page === "snapshot" ? "Import or export Registry snapshots and CSV" : "Live qualification is planned"}</TooltipContent></Tooltip><div className="ml-auto flex items-center gap-2"><Badge variant={connection ? "success" : "warning"} className="text-[10px]">{busy || (connection ? `Bifrost ${connection.version}` : "Disconnected")}</Badge>{!standaloneAuthenticated && <a href="/workspace/providers" className="hidden text-xs text-muted-foreground hover:text-foreground sm:inline">Bifrost workspace</a>}<Button variant="ghost" size="icon" aria-label={dark ? "Use light theme" : "Use dark theme"} title={dark ? "Use light theme" : "Use dark theme"} onClick={() => setDark(!dark)}>{dark ? <Sun className="size-4" /> : <Moon className="size-4" />}</Button>{standaloneAuthenticated ? <Button variant="outline" size="icon" className="size-8 shrink-0" aria-label="Sign out" title="Sign out" disabled={!!busy || loading} onClick={requestSignOut}><LogOut className="size-4" /></Button> : null}</div></header>
      <div ref={contentRef} className="custom-scrollbar content-container mx-0 min-h-0 min-w-0 flex-1 overflow-auto border bg-background md:mr-3 md:mb-3 md:rounded-md md:px-10"><main className="mx-auto max-w-[1440px] px-4 py-6 md:px-0 md:py-8">{apiError && <div role="alert" className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-sm border border-destructive/40 bg-destructive/10 p-3 text-sm"><span>{apiError}</span><Button variant="outline" size="sm" onClick={() => void load()}>Reload workspace</Button></div>}
        ...
        {page === "groups" && <><div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">Groups</h2><p className="mt-1 max-w-prose text-sm text-foreground/70">Reusable model selections shared by virtual keys.</p></div><Button data-tour="groups" onClick={() => openGroup({ id: crypto.randomUUID(), name: "", description: "", members: [] })}><Plus className="size-4" />Create group</Button></div><div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-sm border bg-card p-4 shadow-sm"><div className="relative min-w-[220px] max-w-sm flex-1"><Search className="absolute top-2.5 left-3 size-4 text-muted-foreground" /><Input aria-label="Search groups" placeholder="Search groups..." className="pl-9" value={search} onChange={e => setSearch(e.target.value)} /></div><ViewControls value={{...preferences, ...listOverrides["groups"]}} onChange={next => setView("groups", next)} onReset={() => resetView("groups")} scope="Groups" fields={["description", "metadata", "providers"]} /></div>{viewFor("groups").layout === "grid" ? <div className={`grid gap-4 ${viewFor("groups").size === "small" ? "sm:grid-cols-2 xl:grid-cols-4" : viewFor("groups").size === "large" ? "lg:grid-cols-2" : "sm:grid-cols-2 xl:grid-cols-3"}`}>{filteredGroups.map((g, index) => <Card key={g.id} data-tour={index === 0 ? "groups" : undefined} className="gap-0 overflow-hidden py-0"><CardHeader className="border-b bg-muted/40 px-4 py-4 sm:px-6"><CardTitle className="flex items-center gap-2 text-base"><Layers3 className="size-4" />{g.name}</CardTitle></CardHeader><CardContent className="flex flex-1 flex-col gap-3 px-4 py-4 sm:px-6">{viewFor("groups").description && <p className="text-xs text-muted-foreground">{g.description || "No description"}</p>}{viewFor("groups").metadata && <p className="text-base font-semibold">{g.members.length} <span className="text-xs font-normal text-muted-foreground">selected models</span></p>}{viewFor("groups").providers && <p className="text-xs text-muted-foreground">Used by {data.keys.filter(k => k.policy.groups.includes(g.id)).map(k => k.name).join(", ") || "no keys"}</p>}<Button variant="outline" size="sm" className="ml-auto mt-auto w-fit" onClick={() => openGroup(g)}>Edit group <ArrowRight className="size-3.5" /></Button></CardContent></Card>)}</div> : <div className="overflow-hidden rounded-sm border bg-card shadow-sm"><Table className="[&_td]:py-3 [&_td]:whitespace-normal [&_th]:h-12"><TableHeader className="bg-muted/50"><TableRow><TableHead>Group</TableHead><TableHead>Models</TableHead><TableHead>Used by</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{filteredGroups.map(g => <TableRow key={g.id} className="cursor-pointer" onClick={() => openGroup(g)}><TableCell><div className="text-base font-semibold">{g.name}</div><div className="mt-1 text-xs text-muted-foreground">{g.description || "No description"}</div></TableCell><TableCell>{g.members.length} models</TableCell><TableCell className="space-x-1">{data.keys.filter(k => k.policy.groups.includes(g.id)).map(k => <Badge key={k.id} variant="secondary" className="font-mono">{k.name}</Badge>)}{!data.keys.some(k => k.policy.groups.includes(g.id)) && <span className="text-muted-foreground">—</span>}</TableCell><TableCell className="text-right"><Button size="sm" variant="ghost" onClick={e => { e.stopPropagation(); openGroup(g); }}>Edit <ArrowRight className="size-3.5" /></Button></TableCell></TableRow>)}</TableBody></Table></div>}{!filteredGroups.length && <Empty icon={Layers3} title={search ? "No matching groups" : "No groups yet"} detail={search ? "Try another search or clear it." : data.models.length ? "Create a group from registered models, then attach it to a key." : "Register a model first, then create a group and attach it to a key."} action={search ? <Button variant="outline" onClick={() => setSearch("")}>Clear search</Button> : data.models.length ? <Button onClick={() => openGroup({ id: crypto.randomUUID(), name: "", description: "", members: [] })}>Create group</Button> : <Button onClick={() => go("models")}>Browse models</Button>} />}</>}
        {page === "keys" && !key && <><div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">Virtual Keys</h2><p className="mt-1 max-w-prose text-sm text-foreground/70">Create a native key, compose its catalog, and verify the actual /v1/models response.</p></div><Button onClick={() => setKeyForm({ name: "", client: "" })}><Plus className="size-4" />Create key</Button></div><div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-sm border bg-card p-4 shadow-sm"><div className="relative min-w-[220px] max-w-sm flex-1"><Search className="absolute top-2.5 left-3 size-4 text-muted-foreground" /><Input aria-label="Search keys" placeholder="Search keys..." className="pl-9" value={search} onChange={e => setSearch(e.target.value)} /></div><ViewControls value={{...preferences, ...listOverrides["keys"]}} onChange={next => setView("keys", next)} onReset={() => resetView("keys")} scope="Keys" fields={["description", "metadata", "providers"]} /></div>{viewFor("keys").layout === "grid" ? <div className={`grid gap-4 ${viewFor("keys").size === "small" ? "sm:grid-cols-2 xl:grid-cols-4" : viewFor("keys").size === "large" ? "lg:grid-cols-2" : "sm:grid-cols-2 xl:grid-cols-3"}`}>{filteredKeys.map((k, index) => <Card key={k.id} data-tour={index === 0 ? "keys" : undefined} className="gap-0 overflow-hidden py-0"><CardHeader className="border-b bg-muted/40 px-4 py-4 sm:px-6"><CardTitle className="flex items-center gap-2 text-base"><KeyRound className="size-4" />{k.name}</CardTitle></CardHeader><CardContent className="flex flex-1 flex-col gap-3 px-4 py-4 sm:px-6">{viewFor("keys").description && <p className="text-xs text-muted-foreground">Client: {k.client}</p>}{viewFor("keys").metadata && <p className="text-base font-semibold">{k.managed === false ? "Unknown" : members(k.policy, data.groups).length} <span className="text-xs font-normal text-muted-foreground">{k.managed === false ? "native key models" : "selected models"}</span></p>}{viewFor("keys").providers && <p className="text-xs text-muted-foreground">Groups: {k.managed === false ? "Unknown" : k.policy.groups.map(id => data.groups.find(g => g.id === id)?.name).filter(Boolean).join(", ") || "None"}</p>}<div className="mt-auto flex flex-wrap items-center justify-between gap-2 border-t pt-3"><Badge variant={k.managed === false ? "warning" : k.active ? "success" : "secondary"}>{k.managed === false ? "Native · unmanaged" : k.active ? "Active" : "Disabled"}</Badge><Button variant="outline" size="sm" onClick={() => go("keys", k.id)}>Open key <ArrowRight className="size-3.5" /></Button></div></CardContent></Card>)}</div> : <div className="overflow-hidden rounded-sm border bg-card shadow-sm"><Table className="[&_td]:py-3 [&_td]:whitespace-normal [&_th]:h-12"><TableHeader className="bg-muted/50"><TableRow><TableHead>Key</TableHead><TableHead>Client</TableHead><TableHead>Groups</TableHead><TableHead>Models</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{filteredKeys.map((k, index) => <TableRow key={k.id} data-tour={index === 0 ? "keys" : undefined} className="cursor-pointer" onClick={() => go("keys", k.id)}><TableCell className="text-base font-semibold">{k.name}</TableCell><TableCell className="text-muted-foreground">{k.client}</TableCell><TableCell>{k.managed === false ? "Unknown" : k.policy.groups.map(id => data.groups.find(g => g.id === id)?.name).filter(Boolean).join(", ") || "—"}</TableCell><TableCell>{k.managed === false ? "Unknown" : members(k.policy, data.groups).length}</TableCell><TableCell><Badge variant={k.managed === false ? "warning" : k.active ? "success" : "secondary"}>{k.managed === false ? "Native · unmanaged" : k.active ? "Active" : "Disabled"}</Badge></TableCell><TableCell className="text-right"><Button variant="ghost" size="sm" onClick={e => { e.stopPropagation(); go("keys", k.id); }}>Open <ArrowRight className="size-3.5" /></Button></TableCell></TableRow>)}</TableBody></Table></div>}{!filteredKeys.length && <Empty icon={KeyRound} title={search ? "No matching keys" : "No virtual keys yet"} detail={search ? "Try another search or clear it." : "Register a model, optionally make a group, then create a virtual key to compose its catalog."} action={search ? <Button variant="outline" onClick={() => setSearch("")}>Clear search</Button> : <Button onClick={() => setKeyForm({ name: "", client: "" })}>Create key</Button>} />}</>}
        {page === "keys" && key && activePolicy && <><div className="mb-6 flex flex-wrap items-start justify-between gap-4"><div className="min-w-0 max-w-full"><Button variant="ghost" size="sm" className="mb-2 -ml-2" onClick={() => go("keys")}><ArrowLeft className="size-4" />All keys</Button><div className="flex flex-wrap items-center gap-2"><h2 className="min-w-0 break-words text-2xl font-semibold tracking-tight sm:text-3xl">{key.name}</h2><Badge variant={key.managed === false ? "warning" : key.active ? "success" : "secondary"}>{key.managed === false ? "Not configured" : key.active ? "Active" : "Disabled"}</Badge></div><p className="mt-1 text-sm text-foreground/70">{key.client} · Revision <span title={revision}>{revision.slice(0, 12)}</span> · {key.managed === false ? "Native key · Not configured in Registry" : "Registry managed"}</p></div><div className="flex flex-wrap items-center gap-2"><VirtualKeySecret keyId={key.id} disabled={!!busy} onUnauthorized={requireLogin} />{key.managed === false ? <><Badge variant="warning">Not configured</Badge><Button variant="outline" disabled={!!busy} onClick={() => setAdoption({ keyId: key.id, operation: "adopt" })}>Adopt in Registry</Button></> : <><Button variant="outline" disabled={!!busy || isDirty} onClick={() => setAdoption({ keyId: key.id, operation: "rebind" })}>Review binding</Button><Button variant="outline" disabled={!!busy} onClick={() => setRenameForm({ id: key.id, name: key.name })}>Rename</Button><Button variant="outline" disabled={!!busy} onClick={() => { void commit({ ...data, keys: data.keys.map(x => x.id === key.id ? { ...x, active: !x.active } : x) }, key.active ? "Disabling key…" : "Enabling key…", [key.id]); }}>{key.active ? "Disable" : "Enable"}</Button></>}</div></div>{!key.active && key.managed !== false && <p className="mb-5 rounded-sm border border-chart-warning/50 bg-chart-warning/10 p-3 text-sm text-chart-warning-ink">This key is disabled. Clients have no model access until it is enabled; the IDs below describe its saved selection.</p>}
          {key.managed === false ? <Card className="max-w-2xl p-6"><h3 className="text-lg font-semibold">Native key is not managed by Registry</h3><p className="mt-2 text-sm text-muted-foreground">Its native model count is unknown here. Adopt this key to compose a Registry selection; native Bifrost permissions still apply.</p><Button className="mt-4" disabled={!!busy} onClick={() => setAdoption({ keyId: key.id, operation: "adopt" })}>Review adoption</Button></Card> : <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,.8fr)]"><div inert={!!busy} className="min-w-0 space-y-5"><Card className="gap-0 overflow-hidden py-0"><CardHeader className="border-b bg-muted/40 px-4 py-4 sm:px-6"><CardTitle className="text-xl">Compose catalog</CardTitle><p className="text-xs text-muted-foreground">Changes apply only to {key.name}. Shared groups are managed separately.</p></CardHeader><CardContent className="space-y-8 px-4 py-5 sm:px-6"><section data-tour="composer"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h3 className="text-base font-semibold">Shared groups</h3><Button variant="link" size="sm" onClick={() => go("groups")}>Manage groups <ArrowRight className="size-3.5" /></Button></div><div className="grid gap-3 sm:grid-cols-2">{data.groups.map(g => <label key={g.id} className={`flex cursor-pointer items-center gap-3 rounded-sm border p-3 hover:bg-muted/40 ${activePolicy.groups.includes(g.id) ? "border-chart-success/50 bg-chart-success/10" : "bg-card"}`}><Checkbox checked={activePolicy.groups.includes(g.id)} onCheckedChange={checked => setDraft(old => old ? { ...old, groups: checked ? [...old.groups, g.id] : old.groups.filter(x => x !== g.id) } : old)} /><span className="flex-1"><span className="block text-sm font-medium">{g.name}</span><span className="text-xs text-muted-foreground">{g.members.length} models</span></span></label>)}</div>{!data.groups.length && <p className="text-sm text-muted-foreground">No groups yet. Add individual models below or create a group.</p>}</section><section className="border-t pt-6"><h3 className="mb-2 text-base font-semibold">Models and local exceptions</h3><p className="mb-3 text-xs text-muted-foreground">An exclusion here does not change a shared group or another key.</p><p className="mb-2 text-xs text-muted-foreground">{members(activePolicy, data.groups).length} selected · {activePolicy.excluded.length} local exclusions. Selected models remain selected when filters hide them.</p><ModelBrowser models={data.models} selected={members(activePolicy, data.groups)} selectionDetails={id => { const groups = origin(id, activePolicy, data.groups); const excluded = activePolicy.excluded.includes(id); const added = activePolicy.added.includes(id); return { source: excluded ? groups.length ? "From " + groups.join(", ") + " · Excluded locally" : "Excluded locally" : groups.length ? "From " + groups.join(", ") + (added ? " · Added to this key" : "") : added ? "Added to this key" : "Not selected", action: excluded ? groups.length ? "Restore to this key" : "Add to this key" : groups.length ? "Exclude from this key" : added ? "Remove from this key" : "Add to this key" }; }} onToggle={id => setDraft(old => old ? toggleModel(old, id, data.groups) : old)} onSetSelected={ids => setDraft(old => old ? data.models.reduce((policy, model) => members(policy, data.groups).includes(model.id) === ids.includes(model.id) ? policy : toggleModel(policy, model.id, data.groups), old) : old)} preferences={preferences} compact label="Key models" /><div className="mt-2 flex flex-wrap gap-1">{activePolicy.excluded.map(id => <Badge key={id} variant="outline">Excluded locally: {data.models.find(m => m.id === id)?.name || id}</Badge>)}</div></section><section className="border-t pt-6"><h3 className="mb-2 text-base font-semibold">Visible model names</h3><div className="flex flex-wrap gap-2">{formats.map(format => <Button key={format} variant={activePolicy.naming === format ? "default" : "outline"} size="sm" aria-pressed={activePolicy.naming === format} onClick={() => setDraft(old => old ? { ...old, naming: format } : old)}>{format}</Button>)}</div><p className="mt-2 text-xs text-muted-foreground">Short names use the explicitly configured Bifrost route. Multiple accesses do not imply load balancing.</p></section></CardContent></Card><div className="sticky bottom-0 z-10 flex flex-wrap items-center gap-3 rounded-sm border bg-card p-4 shadow-sm"><Button disabled={reconcileBlocked || (!isDirty && !needsReconcile) || !!busy} onClick={publishKey}>{needsReconcile ? "Retry native apply" : "Publish changes"}</Button><Button variant="outline" disabled={!isDirty || !!busy} onClick={() => { setDraft(copy(key.policy)); toast.success("Draft discarded."); }}>Discard draft</Button>{isDirty && <Badge variant="warning" aria-live="polite">{diff.added.length + diff.removed.length ? <>{diff.added.length + diff.removed.length} expected ID changes</> : "Unpublished selection changes"}</Badge>}<span className={`basis-full text-xs font-medium sm:basis-auto ${isDirty ? "text-chart-warning-ink" : "text-muted-foreground"}`}>{reconcileBlocked ? "Reload workspace before another write" : needsReconcile ? "Registry saved; native permissions need reconciliation" : isDirty ? "Unpublished changes" : "Draft matches saved selection"}</span></div></div>
          <div className="min-w-0 space-y-5"><Card data-tour="preview" className="gap-0 overflow-hidden py-0"><CardHeader className="border-b bg-primary/10 px-4 py-4 sm:px-6"><CardTitle className="flex items-center gap-2 text-lg"><Eye className="size-4" />Draft selection preview</CardTitle><p className="text-xs text-muted-foreground">{key.active ? "Draft selection preview" : "Draft selection if enabled"} · {predicted.length} expected IDs</p></CardHeader><CardContent className="px-4 py-4 sm:px-6"><div className="max-h-56 space-y-2 overflow-auto">{predicted.map(id => <div key={id} className="flex min-w-0 items-center justify-between gap-2 rounded-sm border px-3 py-2 font-mono text-xs"><span className="min-w-0 break-all">{id}</span>{diff.added.includes(id) && <Badge variant="success">+ added</Badge>}</div>)}{!predicted.length && <p className="rounded-sm border border-dashed p-6 text-center text-sm text-muted-foreground">This key would see no models.</p>}</div>{diff.removed.length > 0 && <p className="mt-3 text-xs font-medium text-destructive">Removed from saved selection: {diff.removed.join(", ")}</p>}</CardContent></Card><Card className="gap-0 overflow-hidden py-0"><CardHeader className="border-b bg-muted/40 px-4 py-4 sm:px-6"><CardTitle className="flex items-center gap-2 text-lg"><Activity className="size-4" />Saved selection and readback</CardTitle><p className="text-xs text-muted-foreground">The saved selection is the comparison baseline. The last server readback is separate.</p></CardHeader><CardContent className="px-4 py-4 sm:px-6"><div className="mb-4 flex flex-wrap items-center gap-2"><Badge variant={key.publication?.state === "verified" ? "success" : "warning"}>{key.publication?.state === "verified" ? "Verified" : key.publication?.state === "drift" ? "Drift detected" : "Not verified"}</Badge><Button variant="outline" size="sm" disabled={!!busy} onClick={reread}><RotateCcw className="size-3.5" />Read again</Button><Button variant="ghost" size="sm" disabled={!key.publication?.actual} onClick={downloadJson}><FileJson className="size-3.5" />Export JSON</Button></div><p className="mb-2 text-xs text-muted-foreground">Saved selection IDs{!key.active ? " (not accessible while disabled)" : ""}: {savedSelection.join(", ") || "None"}</p><p className="mb-3 break-all text-xs text-muted-foreground">{key.publication?.checkedAt ? `Checked ${new Date(key.publication.checkedAt).toLocaleString()} · revision ${key.publication.revision.slice(0, 12)}` : "No readback recorded for this revision."}</p>{key.publication?.actual && <p className="mb-3 break-all text-xs text-muted-foreground">Last successful response: {key.publication.observedAt ? new Date(key.publication.observedAt).toLocaleString() : "time unavailable"} · observed revision {key.publication.observedRevision?.slice(0, 12) || "unknown"}</p>}{key.publication?.error && <p role="alert" className="mb-3 text-xs text-chart-warning-ink">{key.publication.error}</p>}{key.publication?.state === "drift" && <p className="mb-3 text-xs text-destructive">Missing: {key.publication.missing.join(", ") || "none"} · Unexpected: {key.publication.unexpected.join(", ") || "none"}</p>}<div className="max-h-44 space-y-2 overflow-auto">{observed.map(id => <div key={id} className="break-all rounded-sm border px-3 py-2 font-mono text-xs">{id}</div>)}{!observed.length && <p className="text-sm text-muted-foreground">{key.publication?.actual ? "No models in the last response." : "No successful readback available."}</p>}</div>{key.publication?.actual && <><Button variant="link" size="sm" className="mt-2 px-0" onClick={() => setShowJson(!showJson)}>{showJson ? "Hide" : "Show"} response JSON</Button>{showJson && <pre className="max-h-56 overflow-auto rounded-sm bg-muted p-3 text-[11px]">{JSON.stringify({ object: "list", data: observed.map(id => ({ id, object: "model" })) }, null, 2)}</pre>}</>}</CardContent></Card></div></div>}</>}
    ...
    } }); }}>Delete</Button>}<div className="flex flex-wrap gap-2 sm:ml-auto"><Button variant="outline" onClick={closeGroup}>Cancel</Button><Button disabled={!!busy} onClick={publishGroup}>Publish group</Button></div></SheetFooter></SheetContent></Sheet>
    <Dialog open={!!createdSecret} onOpenChange={open => { if (!open) { setCreatedSecret(""); setCreatedSecretError(""); } }}><DialogContent><DialogHeader><DialogTitle>Copy the virtual key secret</DialogTitle><DialogDescription>This native Bifrost secret is shown once. Store it before closing.</DialogDescription></DialogHeader><Input ref={createdSecretInput} readOnly value={createdSecret} aria-label="Virtual key secret" autoComplete="off" spellCheck={false} className="font-mono text-xs" />{createdSecretError && <p role="alert" className="text-sm text-destructive">{createdSecretError}</p>}<DialogFooter><Button onClick={() => void copyCreatedSecret()}>Copy secret</Button><Button variant="outline" onClick={() => { setCreatedSecret(""); setCreatedSecretError(""); }}>Done</Button></DialogFooter></DialogContent></Dialog>
