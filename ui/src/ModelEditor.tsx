...
import { applyModelId, changeAccessProvider, modelEditorOptions, prefillFromReference, type AccessCandidate } from "./model-editor-data";
...
export default function ModelEditor({ draft, onChange, creating, workspace, error, busy, onSave, onCancel, onDelete, onUnauthorized, actionSlot }: {
  ...
  const changeAccess = (index: number, patch: Partial<Access>) => update({ accesses: draft.accesses.map((access, at) => at !== index ? access : patch.provider !== undefined ? changeAccessProvider({ ...access, ...patch }, draft.id, patch.provider) : { ...access, ...patch }) });
  ...
  return <SheetContent inert={busy} className="min-h-0 max-w-[calc(100vw-1rem)] p-4 sm:p-6">
    ...
    <div className="custom-scrollbar min-h-0 flex-1 space-y-6 overflow-y-auto overscroll-contain pr-2 pb-4">
      <section className="space-y-4" aria-label="Model identity">
