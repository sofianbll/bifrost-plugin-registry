...
export default function CatalogMetadata({ onUnauthorized, onChanged, focus }: { onUnauthorized: () => void; onChanged: () => void; focus?: { target: "reference" | "access"; id: string } | null }) {
  ...
  const load = async () => {
    setBusy(true);
    try { const next = await getCatalog(); setCatalog(next); if (focus?.target === "access") setMatchId(next.accesses.find(access => access.id === focus.id)?.referenceId || ""); setConflict(false); setError(""); setEditError(""); }
  ...
  const mutate = async (work: (revision: string) => Promise<Catalog>, done?: () => void) => {
    ...
    catch (cause) { report(cause); if (edit) setEditError(cause instanceof ApiError && cause.status === 409 ? "The catalogue changed. Reload its revision, review your draft, then save again." : cause instanceof Error ? cause.message : "Could not save correction."); }
    finally { setBusy(false); }
  };
  ...
  useEffect(() => { if (selectedId && catalog && window.innerWidth < 1024) requestAnimationFrame(() => detailRef.current?.scrollIntoView({ block: "start", behavior: "smooth" })); }, [selectedId, catalog]);
  ...
  const saveEdit = () => {
    if (!edit) return;
    let value: unknown;
    try { value = parseCatalogValue(edit.field, edit.value); } catch (cause) { setEditError((cause as Error).message); return; }
  ...
  return <div className="space-y-5">
    ...
    <Dialog open={!!edit} onOpenChange={open => { if (!open && !busy) { setEdit(null); setEditError(""); } }}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Edit {edit?.field.replaceAll("_", " ")}</DialogTitle><DialogDescription>Save a manual correction for this {edit?.target === "reference" ? "reference" : "provider access"}. It stays in place when sources refresh.</DialogDescription></DialogHeader>{edit && <div className="space-y-2"><label htmlFor="catalog-field-value" className="text-sm font-medium">{edit.field.replaceAll("_", " ")}</label>{fieldTypes[edit.field] === "boolean" ? <select id="catalog-field-value" className="flex h-9 w-full rounded-sm border bg-background px-2 text-sm" value={edit.value} onChange={event => { setEdit({ ...edit, value: event.target.value }); setEditError(""); }}><option value="false">False</option><option value="true">True</option></select> : ["array", "object", "json"].includes(fieldTypes[edit.field]) ? <Textarea id="catalog-field-value" value={edit.value} onChange={event => { setEdit({ ...edit, value: event.target.value }); setEditError(""); }} spellCheck={false} /> : <Input id="catalog-field-value" type={fieldTypes[edit.field] === "number" ? "number" : "text"} value={edit.value} onChange={event => { setEdit({ ...edit, value: event.target.value }); setEditError(""); }} spellCheck={false} />}{["array", "object", "json"].includes(fieldTypes[edit.field]) && <p className="text-xs text-muted-foreground">Enter a JSON {fieldTypes[edit.field] === "array" ? "array of strings" : fieldTypes[edit.field] === "json" ? "object or array" : "object"}.</p>}{editError && <p role="alert" className="text-sm text-destructive">{editError}</p>}</div>}<DialogFooter>{conflict && <Button variant="outline" disabled={busy} onClick={() => void load()}>Reload revision</Button>}<Button variant="outline" disabled={busy} onClick={() => { setEdit(null); setEditError(""); }}>Cancel</Button><Button disabled={busy || conflict} onClick={saveEdit}>Save correction</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}
