...
export function ViewControls({ value, onChange, onReset, scope, fields = ["description", "metadata", "providers", "evidence"] }: { value: ViewOptions; onChange: (next: ViewOptions) => void; onReset?: () => void; scope?: string; fields?: ("description" | "metadata" | "providers" | "evidence")[] }) {
  ...
  return <div className="flex flex-wrap items-center gap-1" aria-label={`${scope || "List"} view options`}>
    ...
    <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><Button size="sm" variant="outline"><Settings2 className="size-4" />View options</Button></PopoverTrigger><PopoverContent align="end" className="w-64 space-y-4"><h3 className="border-b pb-3 text-base font-semibold">{scope || "View"} options</h3>
      <label className="block text-xs font-medium text-muted-foreground">Card size<Select value={value.size} onValueChange={size => onChange({ ...value, size: size as ViewOptions["size"] })}><SelectTrigger className="mt-2 w-full text-foreground" aria-label="Card size"><SelectValue /></SelectTrigger><SelectContent>{["small", "medium", "large"].map(size => <SelectItem value={size} key={size}>{size}</SelectItem>)}</SelectContent></Select></label>
      <div className="space-y-3 border-t pt-3"><p className="text-xs font-medium text-muted-foreground">Visible details</p>{fields.map(field => <label key={field} className="flex items-center gap-2 text-sm capitalize"><Checkbox checked={value[field]} onCheckedChange={checked => onChange({ ...value, [field]: checked === true })} />{field}</label>)}</div>
      {onReset && <Button variant="ghost" size="sm" className="w-full border-t" onClick={() => { onReset(); setOpen(false); }}><RotateCcw className="size-3.5" />Reset view override</Button>}
