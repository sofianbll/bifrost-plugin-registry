...
import { ArrowRight, Braces, Check, CircleHelp, FlaskConical, ImagePlus, MessageSquare, Network, Pause, Play, Radio, RotateCcw, ScanEye, Search, Wrench, X } from "lucide-react";
...
import { emptySelection, limitCells, planCells, resolveTargets, selectedSuiteIds, suiteForScenario, suites, updateViewOverride, type Cell, type LabSelection } from "./lab";
...
const suiteIcons = { chat: MessageSquare, stream: Radio, tools: Wrench, vision: ScanEye, json: Braces, embedding: Network, image: ImagePlus, "responses-chat": MessageSquare, "responses-stream": Radio, "responses-tools": Wrench, "messages-chat": MessageSquare, "messages-stream": Radio, "messages-tools": Wrench };
...
export default function Laboratory({ models, groups, campaigns, onCampaigns, preferences = defaultViewOptions }: Props) {
  const selectSuitesId = useId();
  const topRef = useRef<HTMLDivElement | null>(null);
  const [selection, setSelection] = useState<LabSelection>(emptySelection);
  const [suiteMode, setSuiteMode] = useState<"all" | "custom">("all");
  const [customSuiteIds, setCustomSuiteIds] = useState<string[]>(() => suites.map(suite => suite.id));
  ...
  const [step, setStep] = useState<"tests" | "models" | "review">("tests");
  ...
  const suiteIds = selectedSuiteIds(suiteMode, customSuiteIds);
  ...
  const visibleSuiteIds = visibleSuites.map(suite => suite.id);
  const suiteCounts = selectionCounts(customSuiteIds, visibleSuiteIds);
  const excluded = allAccesses.filter(row => selection.excludedAccessIds.includes(row.access.id));

  useEffect(() => { topRef.current?.scrollIntoView({ block: "start" }); }, [step, view]);
  ...
  const rerunUnresolved = (rows: Campaign[]) => {
    ...
    runRef.current = next;
    setReport(null); setView("new"); setStep("review"); setRun(next);
  };

  return <div ref={topRef} className="space-y-6">
    ...
    <div className="flex flex-wrap gap-2 border-b pb-2" aria-label="Laboratory views">
      <Button aria-pressed={view === "new"} variant={view === "new" ? "secondary" : "ghost"} onClick={() => setView("new")}>New run</Button>
      <Button aria-pressed={view === "history"} variant={view === "history" ? "secondary" : "ghost"} onClick={() => setView("history")}>History <Badge variant="outline">{campaigns.length}</Badge></Button>
    </div>
    ...
    {view === "new" && <>
      <nav className="flex flex-wrap gap-2" aria-label="New run steps">
        {([ ["tests", "1 · Tests"], ["models", "2 · Models & accesses"], ["review", "3 · Review"] ] as const).map(([value, label]) => <Button key={value} size="sm" aria-pressed={step === value} variant={step === value ? "secondary" : "outline"} onClick={() => setStep(value)}>{label}</Button>)}
      </nav>
      {step === "tests" && <section className="space-y-4" aria-label="Select tests">
        <div><h3 className="text-sm font-semibold">Choose tests</h3><p className="text-xs text-muted-foreground">{suites.length} prototype scenarios inspired by Bifrost · full official collection not imported · no live execution.</p></div>
        <div className="flex flex-wrap gap-2" role="group" aria-label="Test selection mode">
          <Button aria-pressed={suiteMode === "all"} variant={suiteMode === "all" ? "secondary" : "outline"} onClick={() => setSuiteMode("all")}>All tests</Button>
          <Button aria-pressed={suiteMode === "custom"} variant={suiteMode === "custom" ? "secondary" : "outline"} onClick={() => setSuiteMode("custom")}>Custom selection</Button>
          <Badge variant="outline" className="self-center">{suiteIds.length} / {suites.length} selected</Badge>
        </div>
        {suiteMode === "custom" && <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2"><div className="relative min-w-48 flex-1"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input aria-label="Search tests" placeholder="Search tests or endpoints" value={suiteSearch} onChange={event => setSuiteSearch(event.target.value)} className="pl-9" /></div><Select value={useFilter} onValueChange={setUseFilter}><SelectTrigger aria-label="Filter by use" className="min-w-36"><SelectValue /></SelectTrigger><SelectContent>{["All uses", ...categories].map(value => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select><div className="flex items-center gap-2 rounded-sm border px-3 py-2 text-xs"><Checkbox id={selectSuitesId} disabled={!suiteCounts.visibleTotal} checked={suiteCounts.visibleSelected === 0 ? false : suiteCounts.visibleSelected === suiteCounts.visibleTotal ? true : "indeterminate"} onCheckedChange={() => setCustomSuiteIds(old => setVisibleSelection(old, visibleSuiteIds, selectionCounts(old, visibleSuiteIds).visibleSelected !== visibleSuiteIds.length))} /><label htmlFor={selectSuitesId} className={suiteCounts.visibleTotal ? "cursor-pointer font-medium" : "text-muted-foreground"}>{suiteCounts.visibleSelected === suiteCounts.visibleTotal && suiteCounts.visibleTotal ? `Deselect all ${suiteCounts.visibleTotal} visible tests` : `Select all ${suiteCounts.visibleTotal} visible tests`}</label><span className="text-muted-foreground">{suiteCounts.visibleSelected}/{suiteCounts.visibleTotal} visible · {suiteCounts.totalSelected} total selected{suiteCounts.hiddenSelected ? ` · ${suiteCounts.hiddenSelected} outside results` : ""}</span></div><ViewControls value={suiteView} onChange={next => setSuiteOverride(old => updateViewOverride(preferences, old, next))} onReset={() => setSuiteOverride({})} scope="Test catalog" fields={["description", "metadata", "providers", "evidence"]} /></div>
      {suiteView.layout === "grid" ? <div className={`grid gap-3 ${suiteView.size === "small" ? "sm:grid-cols-2 xl:grid-cols-4" : suiteView.size === "large" ? "lg:grid-cols-2" : "sm:grid-cols-2 xl:grid-cols-3"}`}>{visibleSuites.map(suite => <Card key={suite.id} className="gap-3 py-4"><CardHeader className="px-4"><div className="flex items-start gap-3"><Checkbox id={`suite-${suite.id}`} checked={customSuiteIds.includes(suite.id)} onCheckedChange={() => setCustomSuiteIds(old => toggle(old, suite.id))} aria-label={`Select ${suite.name}`} /><div className="min-w-0"><label htmlFor={`suite-${suite.id}`} className="flex cursor-pointer items-center gap-2 text-sm font-semibold"><SuiteIcon id={suite.id} />{suite.name}</label><p className="mt-1 text-xs text-muted-foreground">{suite.use}</p></div></div></CardHeader><CardContent className="space-y-2 px-4">{suiteView.metadata && <code className="block break-all rounded-sm bg-muted px-2 py-1 text-xs">{suite.endpoint}</code>}{suiteView.description && <p className="text-xs text-muted-foreground">{suite.criterion}</p>}{(suiteView.providers || suiteView.evidence) && <details className="text-xs text-muted-foreground"><summary className="cursor-pointer">Coverage details</summary><div className="mt-2 flex flex-wrap gap-1">{suiteView.providers && surfaceNames(suite).map(name => <Badge variant="outline" key={name}>{name}</Badge>)}{suiteView.evidence && <Badge variant="secondary">Illustrative mapping · model unverified</Badge>}</div>{suite.coverageNote && <p className="mt-2">{suite.coverageNote}</p>}</details>}</CardContent></Card>)}</div> : <div className="overflow-auto rounded-sm border"><Table><TableHeader><TableRow><TableHead>Select</TableHead><TableHead>Scenario</TableHead><TableHead>Use</TableHead>{suiteView.metadata && <TableHead>Endpoint</TableHead>}{suiteView.providers && <TableHead>Provider surfaces</TableHead>}</TableRow></TableHeader><TableBody>{visibleSuites.map(suite => <TableRow key={suite.id}><TableCell><Checkbox aria-label={`Select ${suite.name}`} checked={customSuiteIds.includes(suite.id)} onCheckedChange={() => setCustomSuiteIds(old => toggle(old, suite.id))} /></TableCell><TableCell className="font-medium"><span className="flex items-center gap-2"><SuiteIcon id={suite.id} />{suite.name}</span>{suiteView.description && <p className="text-xs font-normal text-muted-foreground">{suite.criterion}</p>}{suiteView.evidence && <p className="text-xs font-normal text-muted-foreground">{suite.coverageNote || "Illustrative mapping · model unverified"}</p>}</TableCell><TableCell>{suite.use}</TableCell>{suiteView.metadata && <TableCell className="font-mono text-xs">{suite.endpoint}</TableCell>}{suiteView.providers && <TableCell className="text-xs">{surfaceNames(suite).join(", ")}</TableCell>}</TableRow>)}</TableBody></Table></div>}
      {!visibleSuites.length && <p className="rounded-sm border p-5 text-sm text-muted-foreground">No matching scenarios.</p>}
        </div>}
        <div className="flex justify-end"><Button onClick={() => setStep("models")} disabled={!suiteIds.length}>Choose models & accesses <ArrowRight className="size-4" /></Button></div>
      </section>}
      {step === "models" && <section className="space-y-4" aria-label="Select models and accesses">
        <p className="text-sm text-muted-foreground">{new Set(targets.map(target => target.model.id)).size} models · {targets.length} included accesses · {excluded.length} excluded</p>
        <details className="rounded-sm border p-3"><summary className="cursor-pointer text-sm font-medium">Provider, group & creator shortcuts · {selection.providerIds.length + selection.groupIds.length + selection.creators.length} selected</summary><div className="mt-3">      <Card className="gap-4 py-4"><CardHeader><CardTitle className="text-sm">Expand by provider, group, or creator</CardTitle><p className="text-xs text-muted-foreground">Each choice adds accesses. Exclusions below always win.</p></CardHeader><CardContent className="space-y-4">{([["Providers", "providerIds", providers], ["Groups", "groupIds", groups.map(group => group.id)], ["Creators", "creators", creators]] as const).map(([label, key, values]) => <div key={key}><h4 className="mb-2 text-xs font-medium text-muted-foreground">{label}</h4><div className="flex flex-wrap gap-1.5">{values.map(value => <Button key={value} size="sm" variant={selection[key].includes(value) ? "secondary" : "outline"} aria-pressed={selection[key].includes(value)} onClick={() => setScope(key, value)}>{selection[key].includes(value) && <Check className="size-3" />}{key === "groupIds" ? groups.find(group => group.id === value)?.name : key === "providerIds" ? displayProvider(value) : value}</Button>)}</div></div>)}</CardContent></Card></div></details>
        <details className="rounded-sm border p-3"><summary className="cursor-pointer text-sm font-medium">Exact accesses & exclusions · {targets.length} included, {excluded.length} excluded</summary><div className="mt-3">      <Card className="gap-4 py-4"><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><CardTitle className="text-sm">Resolve exact accesses</CardTitle><Badge variant="secondary">{new Set(targets.map(target => target.model.id)).size} models · {targets.length} accesses · {excluded.length} excluded</Badge></div><p className="text-xs text-muted-foreground">The access ID below is retained exactly in every report, including custom IDs.</p></CardHeader><CardContent>{targets.length ? <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{targets.map(({ model, access }) => <div key={`${model.id}:${access.id}`} className="flex min-w-0 items-start justify-between gap-2 rounded-sm border p-3"><div className="min-w-0"><p className="truncate text-sm font-medium">{model.name}</p><p className="break-all font-mono text-xs text-muted-foreground">{access.id}</p><Badge variant={access.status === "Configured" ? "secondary" : "warning"} className="mt-2">{access.status}</Badge></div><Button variant="ghost" size="icon" aria-label={`Exclude ${access.id}`} title="Exclude access" onClick={() => setSelection(old => ({ ...old, excludedAccessIds: [...old.excludedAccessIds, access.id] }))}><X className="size-4" /></Button></div>)}</div> : <p className="text-sm text-muted-foreground">Choose a model, provider, group, or creator to resolve accesses.</p>}{excluded.length > 0 && <div className="mt-3 flex flex-wrap items-center gap-1.5"><span className="text-xs text-muted-foreground">Excluded:</span>{excluded.map(({ access }) => <Button key={access.id} variant="outline" size="sm" onClick={() => setSelection(old => ({ ...old, excludedAccessIds: old.excludedAccessIds.filter(id => id !== access.id) }))}><span className="max-w-48 truncate font-mono text-xs">{access.id}</span><RotateCcw className="size-3" /></Button>)}</div>}</CardContent></Card></div></details>
      <Card className="gap-4 py-4"><CardHeader><CardTitle className="text-sm">Select models</CardTitle><p className="text-xs text-muted-foreground">Checkbox counts here show direct picks only. Other scopes can add more accesses. Each model includes its listed accesses; unconfirmed ones are skipped.</p></CardHeader><CardContent><ModelBrowser models={models} selected={selection.modelIds} onToggle={id => setScope("modelIds", id)} onSetSelected={ids => setSelection(old => ({ ...old, modelIds: ids }))} preferences={preferences} label="Direct model picks" /></CardContent></Card>
        <div className="flex flex-wrap justify-between gap-2"><Button variant="outline" onClick={() => setStep("tests")}>Back to tests</Button><Button onClick={() => setStep("review")} disabled={!targets.length}>Review batch <ArrowRight className="size-4" /></Button></div>
      </section>}
      {step === "review" && <section className="space-y-4" aria-label="Review batch">
        <p className="text-sm text-muted-foreground">{suiteIds.length} tests · {new Set(targets.map(target => target.model.id)).size} models · {targets.length} accesses · {cells.length} cases{selection.excludedAccessIds.length > 0 && ` · ${selection.excludedAccessIds.length} excluded accesses`}</p>
      <Card className="gap-4 py-4"><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><CardTitle className="text-sm">Review batch</CardTitle><span className="flex gap-1"><Badge variant="secondary">{runnable} runnable</Badge><Badge variant="outline">{skipped} skipped</Badge><Badge variant="outline">{cells.length} cases</Badge></span></div><p className="text-xs text-muted-foreground">Runnable means this illustrative provider mapping and declared tasks/modalities fit. It does not confirm capability.</p></CardHeader><CardContent className="space-y-4"><details className="rounded-sm border p-3"><summary className="cursor-pointer text-sm font-medium">Advanced run controls</summary><div className="mt-3 flex flex-wrap gap-4"><label className="text-xs">Concurrent fixture steps<Select value={String(concurrency)} onValueChange={value => setConcurrency(Number(value))}><SelectTrigger className="mt-1 min-w-24" aria-label="Concurrent fixture steps"><SelectValue /></SelectTrigger><SelectContent>{[1, 2, 4].map(value => <SelectItem key={value} value={String(value)}>{value}</SelectItem>)}</SelectContent></Select></label><label className="text-xs">Case ceiling<Input className="mt-1 w-28" type="number" min={1} max={100} value={caseCeiling} onChange={event => setCaseCeiling(Math.max(1, Math.min(100, Number(event.target.value) || 1)))} /></label></div><p className="mt-2 text-xs text-muted-foreground">Controls affect only this local fixture run. The ceiling turns excess scenario-access cases into skipped results.</p></details><details className="rounded-sm border p-3"><summary className="cursor-pointer text-sm font-medium">Case matrix · {cells.length} cases</summary><div className="mt-3 max-h-80 overflow-auto rounded-sm border"><Table><TableHeader><TableRow><TableHead>Exact access</TableHead><TableHead>Scenario</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{cells.map((cell, index) => <TableRow key={`${cell.target.access.id}:${cell.suite.id}:${index}`}><TableCell className="break-all font-mono text-xs">{cell.target.access.id}</TableCell><TableCell className="text-xs">{cell.suite.name}</TableCell><TableCell><Badge variant={cell.runnable ? "secondary" : "outline"}>{cell.runnable ? "Runnable" : "Skip"}</Badge><span className="ml-2 text-xs text-muted-foreground">{cell.reason}</span></TableCell></TableRow>)}</TableBody></Table></div></details>{!cells.length && <p className="text-sm text-muted-foreground">Select at least one scenario and access to preview a batch.</p>}<div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs text-muted-foreground">No API calls or costs. All outcomes are fixed local fixtures.</p><Button disabled={!runnable || !!run} onClick={start}><Play className="size-4" />Run fixture batch</Button></div></CardContent></Card>
        <details className="rounded-sm border p-3 text-xs text-muted-foreground"><summary className="cursor-pointer text-sm font-medium text-foreground">Optional Bifrost plugins</summary><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><strong className="text-foreground">Mocker</strong><p>Controlled dummy responses can check a mocked flow, never real provider capability. Not connected here.</p><a className="text-primary underline" href={mockerUrl} target="_blank" rel="noreferrer">Bifrost docs ↗</a></div><div><strong className="text-foreground">JSON Parser</strong><p>Repairs incomplete JSON chunks while streaming. It does not prove structured output support. Not connected here.</p><a className="text-primary underline" href={parserUrl} target="_blank" rel="noreferrer">Bifrost docs ↗</a></div></div></details>
        <Button variant="outline" onClick={() => setStep("models")}>Back to models & accesses</Button>
      </section>}
    </>}
    {view === "history" && <>
