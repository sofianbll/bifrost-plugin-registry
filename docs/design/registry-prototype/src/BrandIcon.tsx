...
export function BrandIcon({ model, mode, className }: { model: Model; mode: ViewOptions["logo"]; className?: string }) {
  return <span className={cn("flex shrink-0 items-center", className)}>{mode === "creator" ? <Logo icon={creatorLogo[model.creator]} image={model.creator === "Google" ? "/images/google.svg" : undefined} label={model.creator} /> : <span className="flex items-center -space-x-2">{[...new Set(model.accesses.map(a => a.provider))].map(id => <Logo key={id} icon={providerLogo[id]} label={displayProvider(id)} />)}</span>}</span>;
}
