...
- Première validation bloquante : sur une instance isolée, mesurer `GET /v1/models` avec deux clés distinctes, le filtrage des modèles cachés, les noms, l'identité transmise au hook, l'intersection avec les droits natifs et le rafraîchissement des caches. Tester aussi une clé existante sans politique pour choisir son traitement avant tout branchement global.
...
Le prototype React est une référence d'expérience, pas une preuve de runtime. Sur le code actuel, les tests de projection et de refus passent : le hook natif projette la réponse après succès natif avec une identité de gouvernance obligatoire, en ne gardant que les routes déjà présentes. Cela établit le mécanisme de filtrage dans le plugin, **pas** son comportement HTTP par clé virtuelle sur l'instance Bifrost cible ; les anciens essais concernent surtout l'inférence. La version actuelle du moteur, du store et de l'admin est décrite dans [STATUS](../../STATUS.md) ; le vocabulaire est dans [CONTEXT](../../CONTEXT.md), les décisions de produit et limites dans [product-direction](product-direction.md). Après résolution des arbitrages, les tickets pourront porter des dépendances et preuves de fin distinctes. Cette spec est un cadrage à finaliser, pas une autorisation de déploiement.
