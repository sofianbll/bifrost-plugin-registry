...
La [spec V1](core-v1-spec.md) fixe le comportement du catalogue, des groupes et des clés ; ses anciens critères d'UI native restent historiques. Le candidat final possède un catalogue central enrichi par Bifrost, Models.dev et les données utiles de la première V1, avec provenance par champ, corrections manuelles, dernier état valide, import/export JSON versionné et CSV aplati. L'assistance IA de correspondance reste optionnelle et ultérieure ; le laboratoire demeure différé.

Le mode standard de [#8](https://github.com/sofianbll/bifrost-plugin-registry/issues/8) est **implémenté et qualifié localement** : le `.so` embarque l'UI React et sert son panneau sur `8099`. Il est séparé de l'image Bifrost 2.2.2 compilée avec liaison dynamique depuis les sources officielles non modifiées. Les [preuves finales ARM64](../../reports/v1-final/arm64/) passent 42/42 contrôles de modèles, 54/54 du plugin et 26/26 de l'image avec mise à jour/retour arrière ; les [preuves AMD64](../../reports/v1-final/amd64/) passent 42/42, 54/54 et 18/18. ARM64 ajoute [19/19 contrôles d'adoption](../../reports/v1-final/arm64/adoption-report.json) et [4/4 du client Hermes installé](../../reports/v1-final/arm64/hermes/report.json), avec fournisseur synthétique. La [revue navigateur](../../reports/v1-final/README.md#revue-navigateur) est consignée. La [prérelease v0.2.0-rc.1](https://github.com/sofianbll/bifrost-plugin-registry/releases/tag/v0.2.0-rc.1) distribue les artefacts ; la production n’a pas été modifiée.

Deux modes d'installation sont retenus, sans supprimer le travail existant :

| Mode | Priorité et prérequis |
| --- | --- |
| **Standard** | Prioritaire : image Bifrost dynamique et plugin compilés ensemble mais distribués séparément, URL directe du `.so`, panneau autonome, sans patch hôte. La réactivation à chaud échoue avec `plugin already loaded` ; le redémarrage restaure le panneau et les données. Distribution versionnée dans `v0.2.0-rc.1`. |
| **Intégré** | Différé : pilote avec routes, menu et authentification Bifrost communs. Ses sources, patchs, binaires et preuves restent archivés ; [#7](https://github.com/sofianbll/bifrost-plugin-registry/issues/7) conserve ce jalon historique. |

Dans Bifrost 2.2.2 étudié, le contrat standard des plugins ne permet pas d'ajouter une route au serveur HTTP Bifrost. Le plugin sert donc l'UI React et son API sur son propre port, avec un jeton admin distinct, et appelle l'API native du gateway sur `8080`. Un reverse proxy externe peut présenter l'UI sous un domaine ou un chemin, avec une configuration réseau et d'accès propre au déploiement ; ce parcours externe n'est pas qualifié ici. Voir [la procédure de livraison](../RELEASE.md) et [l'audit des surfaces](stock-plugin-install-research.md).

L'UI Registry reste dans le produit. La [stratégie du fork personnel](bifrost-fork-strategy.md) conserve l'intégration native comme option ultérieure. L'[image officielle précompilée 2.2.2 ARM64 testée](../../reports/stock-v2.2.2-plugin-install/README.md) refuse toujours le chargement dynamique. Les preuves finales qualifient notre image dynamique et un parcours Hermes sur fixture, sans inférence fournisseur réelle ni déploiement de production. La spec V1 conserve des critères d'UI native historiques ; #7 est différé.
