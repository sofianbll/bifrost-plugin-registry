...
python3 - "$ROOT" "$STAGE" "$MODULE" <<'PY'
import pathlib,shutil,sys
root,stage=map(pathlib.Path,sys.argv[1:3]); module=sys.argv[3]
...
p=pathlib.Path(sys.argv[1]);names=['bifrost-http','bifrost-registry.so','native-probe']
